...
def obter_mototaxis_disponiveis(cliente_x, cliente_y):
    ...
    cursor.execute("SELECT id, latitude, longitude FROM entregadores WHERE disponivel = true")
...
def callback(ch, method, properties, body):
    ...
    if partes[0] == "RESPOSTA":
        ...
        if status == "ACEITO":
            print("Seu entregador está a caminho!")
        elif status == "RECUSADO":
            print("Desculpe, o entregador não pode aceitar a solicitação.")
        else:
...
def aguardar_resposta_do_entregador(cliente_id):
    ...
    print('Aguardando resposta do entregador...')
